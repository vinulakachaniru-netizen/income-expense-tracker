    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-800">Set Savings Goal</h3>
            <p className="text-xs text-slate-500">Track progress from your savings</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-slate-600">Goal Name</label>
            <input
              type="text"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder="e.g. Save for a trip"
              className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-800 outline-none transition focus:border-teal focus:ring-2 focus:ring-teal/20"
              required
            />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-slate-600">Target Amount (LKR)</label>
            <input
              type="number"
              value={target}
    ...
    <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-800">{category} — Deep Dive</p>
            <p className="text-xs text-slate-500">{catTxns.length} transactions · {formatLKR(total)}</p>
          </div>
        </div>
        <button
          type="button"
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs text-slate-700 outline-none focus:border-teal focus:ring-1 focus:ring-teal/30"
                />
              </div>
              <div className="flex flex-col gap-1">
        ...
        <div className="mt-5">
          <div className="rounded-xl border border-slate-100 bg-gradient-to-r from-slate-50 to-white p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-slate-700">Top Spending Category</p>
            </div>
            {topCategory ? (
              <>
                <div className="mt-3 flex items-center gap-2">
                  ...
                  />
                  <span className="text-xl font-bold text-slate-800">{topCategory.name}</span>
          ...
          <div className="mt-5 rounded-xl border border-teal/20 bg-teal/5 p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-700">{savingsGoal.label}</p>
              ...
              <span
                className={`rounded-full px-2.5 py-1 text-sm font-bold ${
                  goalPct >= 100 ? "bg-emerald-100 text-emerald-600" : "bg-teal/10 text-teal"
                }`}
              >
                {goalPct}%
              </span>
        ...
        <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-slate-700">Expenses by Category</h3>
            </div>
          ...
          <div>
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-slate-700">Last 7 Days Spending</h3>
            </div>
            {hasDailySpending ? (
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={dailyData} barCategoryGap="20%">
                  ...
                  <Tooltip
                    ...
                    cursor={{ fill: "rgba(0, 109, 119, 0.06)" }}
                  />
                  <Bar
                    dataKey="amount"
                    radius={[6, 6, 0, 0]}
